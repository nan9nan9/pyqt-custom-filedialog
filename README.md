...
열쇠는 `(종류, 심볼릭 링크인가, 확장자)` 이고, **종류는 Qt 가 아이콘을 고를 때
가르는 기준 그대로**입니다 — 뿌리(디스크) · 폴더 · 파일 · 그 밖(FIFO·소켓·장치·
끊긴 링크는 Qt 가 빈 아이콘을 줍니다). 뭉뚱그리면 먼저 물어본 것이 뒤엣것을
덮어씁니다. 실제로 뿌리 `/` 를 폴더와 같은 열쇠로 묶었을 때 **모든 폴더가
하드디스크 모양**이 됐습니다(모델이 시작 폴더의 인덱스를 만들며 루트부터 묻기
때문입니다). 링크 여부를 넣는 이유도 같습니다 — 즐겨찾기·최근 파일 폴더는 안이
전부 링크라 빼먹으면 하필 이 라이브러리가 만드는 화면에서 표시가 사라집니다.

맞바꾼 것은 두 가지입니다.

- 확장자가 없는 **파일**들이 내용과 무관하게 같은 아이콘을 받습니다
  (점파일 `.bashrc` 도 확장자 없음으로 묶입니다).
- **폴더마다 다른 아이콘**을 두는 데스크톱 설정이 무시됩니다.
...
> 세그폴트 위험이 없습니다. GUI 스레드가 한 번에 멈추는 시간도 프로브 몫
> (기본 0.25초) 이내입니다.
