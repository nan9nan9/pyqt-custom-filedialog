...
**폴더를 옮길 때**(사이드바를 누르거나 폴더를 두 번 누를 때)도 잽니다. 여는 것과
따로 재야 하는 구간입니다.

```
DEBUG custom_file_dialog: > 폴더 이동: /user/jekai
DEBUG custom_file_dialog: 폴더 이동 끝: /user/jekai = 842.6 ms
DEBUG custom_file_dialog: 아이콘: 물음 274회 · Qt 조회 10회 · 48.9 ms (이 폴더를 나열하며)
```

시작은 그 폴더로 옮기기로 한 시점, 끝은 **모델이 그 폴더를 다 읽은** 시점입니다
(`directoryLoaded`). 그 사이가 나열에 든 시간이고, 네트워크 폴더에서 눈에 보이는
지연이 그것입니다.

아이콘 줄의 **물음 대 조회**가 핵심입니다. Qt 는 목록의 항목마다(화면에 안 보이는
것까지) 아이콘을 묻고, 답을 만들려면 `~/.icons` 를 뒤집니다 — 홈이 네트워크면 그
하나하나가 서버 왕복입니다(실측 항목당 4.9 ms · 274개에 1.3초). 캐시가 그걸
막으라고 있는데, **그 환경에서 실제로 듣고 있는지**는 이 두 숫자로만 알 수
있습니다. 위 예처럼 `274회 → 10회` 면 잘 듣는 것이고, 둘이 비슷하면 캐시가 죽은
것입니다.

> 아이콘 계수는 **우리 아이콘 제공자가 걸렸을 때만** 나옵니다. 즐겨찾기·최근
> 파일을 쓰지 않으면(`favorites_icon=False` 포함) Qt 기본 제공자가 쓰이고, 그쪽
> 조회는 우리가 셀 수 없습니다.
...
**여는 순간** — 창이 처음 뜰 때까지를 단계로 쪼갭니다. 이 값들은 Qt 가 프로세스
안에 캐시해서 두 번째부터는 0 ms 가 나오므로, **새 프로세스에서 한 번** 잽니다.

```
단계                                                 걸린 시간  몫
파이썬·Qt 모듈 import                                    66 ms  41.3% ##########
QApplication (플랫폼·테마 플러그인 · 폰트)                      3 ms   2.0% #
Qt 설정 읽기 (저장된 사이드바)                                  0 ms   0.2%
표준 위치 (XDG user-dirs)                                0 ms   0.1%
종류 판별 첫 조회 (shared-mime-info)                        1 ms   0.4%
아이콘 테마 첫 조회 (~/.icons · ~/.local/share/icons)        4 ms   2.5% #
맨 QFileDialog 생성                                    21 ms  12.9% ###
맨 QFileDialog show()                                 9 ms   5.6% #
우리 저장소 열기 (즐겨찾기 · 최근 파일)                            29 ms  18.0% #####
CustomFileDialog 생성 (사이드바 · 훅 · 안전 판정)              19 ms  12.0% ###
CustomFileDialog show()                              8 ms   5.0% #

맨 QFileDialog 29 ms  vs  이 라이브러리 56 ms  (차이 +27 ms)
```

50 ms 를 넘은 단계마다 **무엇을 읽는지**와 할 일을 붙여 줍니다 — 아이콘 테마면
`~/.icons`, 표준 위치면 `~/.config/user-dirs.dirs` 처럼, 홈이 네트워크일 때
왕복이 되는 자리들입니다. 맨 `QFileDialog` 와 나란히 찍으므로 **여는 비용의
어디까지가 Qt 자신인지** 바로 보입니다.

**오갈 때** — 이미 열린 창에서 사이드바를 누르는 비용입니다. 클릭 한 번을
**우리 코드**와 **Qt** 로 갈라 재고, 우리 코드 쪽이 크면 어느 함수인지까지
찍습니다. `examples/demo.py` 는 사이드바를 누를 때마다 같은 내용을 아래 로그에
남깁니다.

그 밖에 홈·저장소가 어느 마운트에 얹혀 있는지, 원격 마운트의
`noac`/`actimeo=0`(속성 캐시를 끄는 옵션 — 목록이 느린 흔한 원인), 그리고 Qt 가
사이드바를 저장하는 파일이 비정상적으로 커지지 않았는지도 함께 알려 줍니다.
...
새 분류를 만들면 사이드바에도 바로 나타납니다. 즐겨찾기 분류 폴더 **안의 링크**에는
이 메뉴가 뜨지 않습니다(이미 등록된 것이므로). **최근 파일은 예외로 뜹니다** —
아래를 보십시오.
...
<최근 파일> 폴더에서 우클릭
 ├─ '최근 파일'에서 제거
 ├─ 즐겨찾기에 추가 ▸        ← **원본** 을 등록합니다(링크가 아니라)
 ├─ ─────
 ├─ 경로 복사
 └─ New Folder
```

**최근 파일에서는 즐겨찾기에 추가할 수 있습니다.** 최근 목록은 지나간 자리를
비추는 곳이라, 거기서 쓸 만한 것을 발견하면 그 자리에서 붙잡아 둘 수 있어야
합니다. 등록되는 것은 링크가 아니라 **원본 경로**입니다 — 링크를 등록하면 그
항목이 최근 목록에서 밀려나는 순간 즐겨찾기가 끊어진 링크로 남습니다.

**최근 파일에서는 `Rename` 과 `Show hidden files` 를 붙이지 않습니다.** 최근
목록은 폴더가 아니라 거울이라 두 항목 모두 뜻이 없습니다 — "이름 변경"은 원본이
아니라 거울에 붙은 이름표를 갈 뿐이고(원본은 그대로라, 그 파일을 다시 열면 원래
이름으로 또 쌓입니다), "숨김 파일 표시"가 드러낼 것은 우리가 만든 링크뿐입니다.
비활성으로 두지 않고 아예 뺀 것은, 그 자리에서 **할 수 없는** 동작이 아니라
**뜻이 없는** 동작이기 때문입니다.
...
즐겨찾기·최근 파일 저장소의 기본 자리는 `~/.config` 아래 — 이 라이브러리가
상정하는 **네트워크 홈** 위입니다. 그래서 저장소를 읽는 것도 같은 안전장치를
거칩니다. 홈이 멈추면 **빈 목록으로 물러섭니다** — 사이드바에 분류가 안 보일
뿐 창은 그대로 뜨고, 고른 결과도 그대로 나갑니다.

파일을 고른 **직후**에 자동으로 도는 최근 파일 기록은 저장소가 닿는지 한 번
확인하고 시작합니다. 안쪽에서 하는 일이 여럿이라(있던 링크 찾기 · 링크 만들기 ·
인덱스 쓰기 · 넘친 것 자르기) 저마다 멈추면 그 합만큼 GUI 가 잡히기 때문입니다.

| 멈춘 저장소에서 | 고치기 전 | 지금 |
|---|---|---|
| 다이얼로그 생성 | 9.04초 | 0.28초 |
| 파일 고른 직후 기록 | 18.04초 | 0.00초 |
| 고른 링크를 원본으로 되돌리기 | 18.00초 | 0.00초 |
| 분류 폴더 판정(목록의 항목마다) | 6.00초 | 0.00초 |
| 분류 항목 읽기 | 27.00초 | 0.00초 |
| 이미 등록됐는지 확인 | 9.00초 | 0.00초 |

(`path_timeout=1.0` 을 준 상태에서 잰 값입니다 — 그 약속이 저장소에는 안 걸려
있었습니다.)
...
- `CustomFileDialog` 는 사라질 때 **우리가 얹은 항목만** 사이드바에서 빼고
  닫습니다. Qt 가 전역 설정에 저장하는 목록에 우리 이름이 아예 들어가지
  않습니다. 사용자가 직접 끌어다 놓은 항목이나 앱이 준 항목은 그대로 두고,
  다시 열면 우리 항목도 그대로 돌아옵니다. 다이얼로그가 사라지는 길이 여럿이라
  세 곳에서 지킵니다 — 확정·취소(`done`), 감추기(`hideEvent`), 그리고 **띄운 채
  앱이 끝날 때**(`aboutToQuit`).

  > 남는 한 가지: 이벤트 루프를 정상 종료하지 않고 프로세스가 끝나면(스크립트를
  > 그냥 끝내거나 강제 종료) Qt 가 마지막에 저장하는 것을 막을 방법이 없습니다.
  > 평범한 앱에서는 해당하지 않습니다.
