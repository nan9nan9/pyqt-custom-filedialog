...
CASES = [   (   'history 자르는 기준',
    ...
    (   '넘겨받은 답 쓰기',
        ...
        '        return safety.safe_isdir(absolute)',
        'does_not_restat'),
    (   '종류별 캐시',
        'icons.py',
    ...
    (   '저장소 목록도 안전장치로',
        ...
        '        return _scan_dirs(self.base_dir)',
        'hung_storage_does_not_freeze'),
    (   '최근 목록도 안전장치로',
        'recent.py',
        '        return safety.safe_call(_scan_links, directory, [])',
        '        return _scan_links(directory)',
        'hung_storage_does_not_freeze'),
    (   '기록 전에 도달 확인',
        'recent.py',
    ...
    (   '링크 되돌리기도 안전장치로',
        ...
        '        followed = self._follow_or_none(link)',
        'hung_storage_does_not_freeze'),
    (   '분류 항목 읽기도 안전장치로',
        'favorites.py',
        '        names = safety.safe_call(_scan_names, directory, [])\n        if not names:\n            return []',
        '        names = _scan_names(directory)\n        if not names:\n            return []',
        'hung_storage_does_not_freeze'),
    (   '인덱스 읽기도 안전장치로',
        'favorites.py',
